...
cladelabels(tree,"A",node=28,offset=2)cladelabels(tree,"B",node=29,offset=2)
...
Para ver cómo utilizar el método de contrastes para ajustar un modelo lineal (como un modelo de regresión) en R, primero necesitaremos revisar algunos conceptos básicos sobre cómo ajustar modelos lineales en el entorno informático de R. Podemos comenzar cargando algunos datos sobre el tamaño corporal y el tamaño del área de distribución en varias especies de mamíferos (Garland et al. 1992). Aquí usaremos dos archivos, mamíferoHR.phy y mamíferoHR.csv (disponibles en el proyecto de R).
...
Podemos ver en esta impresión que nuestro marco de datos, mamíferoHR, contiene dos columnas: bodyMass (que proporciona la masa corporal en kilogramos [kg] de cada especie en el conjunto de datos) y homeRange (el tamaño del área de distribución). Los nombres de las filas de nuestro objeto corresponden a los nombres de las especies de cada par de valores. A continuación, podemos seguir adelante y ajustar un modelo en el que el tamaño medio del rango (en km 2 ) varía en función del tamaño corporal total (en kg). Estos caracteres parecen estar correlacionados.
...
Ahora ajustemos un modelo de regresión estándar usando mínimos cuadrados ordinarios (OLS) (Neter et al. 1996). 
Primero ajustamos el modelo a nuestros datos usando una poderosa función llamada lm. Segungo comprobamos la hipótesis. Dada la distribución de nuestras variables, y como es muy común con los datos biológicos, vamos a ajustar nuestro modelo al logaritmo natural del tamaño corporal y al logaritmo natural del tamaño del área de distribución del hogar de la siguiente manera. Esto es muy importante para variables numéricas (como estas) que varían en órdenes de magnitud. La transformación a una escala logarítmica hará que un cambio evolutivo del 10 por ciento en un oso o un ratón sean equivalentes (Glazier 2013). Esto lo haremos con la función log().
...
Para un objeto de lm, el resumen nos da valores P para el modelo y nuestros coeficientes ajustados, r2, F, y muchas de las otras estadísticas que podríamos obtener usando software estadístico estándar para realizar el mismo tipo de análisis. 
...
Luego respresntamos nuestro árbol.
...
La función para calcular contrastes es del paquete ape y se llama pic. Sigamos adelante y calculemos los contrastes para el área de distribución y el tamaño del cuerpo. Haremos esto en dos pasos. Primero, extraeremos vectores separados para cada carácter y asignaremos los nombres de los vectores que corresponden a los nombres de datos mamíferoHR usando la función setNames. A continuación, procederemos a calcular los contrastes para cada uno de estos nuevos vectores con nombre usando la función PIC del paquete APE.
...
El resultado es muy similar al resultado resumido de nuestra regresión ordinaria, siendo la diferencia más significativa que este modelo tiene un solo coeficiente ajustado: la pendiente. 5Esto tiene sentido ya que fijamos el término de intersección del modelo en cero. Finalmente, representemos contrastes independientes y nuestra pendiente ajustada
...
A continuación, simulemos la evolución en este árbol para dos variables, x e y. Tenga en cuenta que vamos a simular los dos caracteres de forma completamente independiente: primero uno y luego el otro, es decir, sin ninguna correlación evolutiva real entre ellos. Después de simular nuestros datos, podemos trazarlos, ajustar un modelo de regresión y graficar ese modelo ajustado sin puntos. En este bloque de código, la función fastBM de phytools se usa para simular la evolución de caracteres independientes para x e y.
...
El r2 de nuestro modelo lineal no es muy alto (sólo alrededor de 0,5), pero la correlación resulta, no obstante, muy significativa. Esto debería ser un poco sorprendente porque, recuerde, estos datos se simularon en ausencia de una correlación evolutiva entre las dos variables. Lo que debemos deducir de este ejemplo es que no es difícil para la filogenia inducir lo que podría caracterizarse como un error de tipo I. Una forma de entender cómo ocurre esto es que en nuestros datos hay grupos de errores estrechamente relacionados (y por lo tanto, estadísticamente no independientes) que tienen fenotipos muy similares, tanto para x como para y, aunque x e y son evolutivamente independientes: es decir, la evolución de x no tuvo ningún efecto sobre la evolución de y o viceversa. Esto se puede visualizar hasta cierto punto usando el método de trazado (filomorfoespacio).
...
Al inspeccionar esta proyección, podríamos empezar a notar que entre especies estrechamente relacionadas, parece haber muy poca correlación entre las dos variables del modelo, lo que sugiere que la aparente correlación podría deberse (como la describe Felsenstein 1985) a las divergencias aleatorias entre un número relativamente pequeño de nodos en el árbol. Ahora veamos si al sustituir los contrastes entre especies (y nodos) por los valores originales en nuestra regresión, resolvemos este error de tipo I. Trazaremos nuestros contrastes y la línea de regresión ajustada.
